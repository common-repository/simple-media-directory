...
class CMB_Image_Field extends CMB_File_Field {
	...
	public function html() {
		...
		*/
		...
		$styles  = sprintf( 'width: %1$dpx; height: %2$dpx; line-height: %2$dpx', intval( 80 ), intval( 80 ) );
		$placeholder_styles  = sprintf( 'width: %dpx; height: %dpx;', intval( 80 ) - 8, intval( 80 ) - 8 );
	...
	public function html() {
		...
		if ( ! empty( $value ) ) {
			foreach ( $value as $field_id => $field_value ) {
				$field_value = ( ! empty( $field_value ) ) ? sanitize_text_field( $field_value )  : array();
