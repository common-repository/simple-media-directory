	...
	class Wp_Usage_Feedback {
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="'.esc_js( "javascript:event.preventDefault();" ).'" id="wpb-goodbye-link-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function form_default_text() {
			...
			$form['body'] = __( 'Before you deactivate the plugin, would you quickly give us your reason for doing so? Only the option you selected in this form will be emailed to us.', 'wpchatbot' );
			$form['options'] = array(
			...
			$form['details'] = __( 'Please provide some details so we can improve the plugin for you (optional)', 'wpchatbot' );
			return $form;
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				...
				foreach( $form['options'] as $option ) {
					$html .= '<input type="checkbox" name="wpb-goodbye-options[]" id="' . str_replace( " ", "", esc_attr( $option ) ) . '" value="' . esc_attr( $option ) . '"> <label for="' . str_replace( " ", "", esc_attr( $option ) ) . '">' . esc_attr( $option ) . '</label><br>';
				}
				$html .= '</p><label for="wpb-goodbye-reasons">' . esc_html( $form['details'] ) .'</label><textarea name="wpb-goodbye-reasons" id="wpb-goodbye-reasons" rows="2" style="width:100%"></textarea>';
			...
			<script>
				jQuery(document).ready(function($){
					$("#wpb-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>").on("click",function(){
						...
						$("#wpb-goodbye-form-<?php echo esc_attr( $this->plugin_name ); ?>").html( '<?php echo $html; ?>' + '<div class="wpb-goodbye-form-footer"><p><a id="wpb-submit-form" class="button primary" href="#">Submit and Deactivate</a>&nbsp;<a class="secondary button" href="'+url+'">Just Deactivate</a></p></div>');
